...
- ROB-115: Updated the context comparison snapshot to include the exact 16384-context standard frequency-mask rows available from `lcasr/results/entropy_ablation/pkl/` for TEDLIUM and Earnings22. These rows use `seq_len=16384`, `overlap=14336`, `epochs=5`, `lr=9e-5`, and frequency masks only (`6` masks, width `34`, no time masks), but remain labelled partial because no matching four-dataset 16384 frequency-mask summary source is committed.
