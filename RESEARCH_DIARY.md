...
- ROB-112: The queued 8192-context no-adapt CTC baseline completed successfully and produced all four expected test-set PKLs. Regenerated `summary.csv`, `summary.md`, `context_baseline_comparison.csv`, and `context_baseline_comparison.md` from artifacts; final WERs are TEDLIUM `6.31%`, Earnings22 `18.68%`, CHiME-6 `84.34%`, and Rev16 `15.17%`. The comparison currently includes 2048, 8192, and 65536 no-adapt rows; the 16384 row remains absent here because ROB-110 tracks that run separately.
