...
- ROB-110: A human follow-up asked whether the thesis-number mismatch could come from `n_seq_sched_16384_rp_2` or `n_seq_sched_16384_rp_3`. Existing committed artifacts did not contain rp2/rp3 no-adapt outputs, so added a callback-backed repeat wrapper that writes rp2/rp3 to separate result roots and a comparison script that contrasts those summaries with the committed rp1 baseline.
...
- ROB-110: The queued Mimas run completed all four 16384-context no-adapt CTC test-set PKLs and regenerated `summary.csv` / `summary.md` plus the 2048/16384/65536 no-adapt comparison table under `lcasr/results/ctc_seq16384_unadapted_baseline/`. Final WERs are 18.29% Earnings22, 6.23% TEDLIUM, 86.52% CHiME-6, and 15.22% Rev16.
- ROB-67: Investigated the 65536-context CTC no-adapt baseline after review concern that the before-adaptation rows looked too close to shorter-context setups. Added `lcasr/results/seq_65536_investigation/investigate_unadapted_baseline.py`, `UNADAPTED_BASELINE_INVESTIGATION.md`, and `unadapted_baseline/investigation.csv`. The check found no launcher or aggregation mismatch; all 65536 PKLs use `epochs=0`, greedy CTC, `seq_len=65536`, `overlap=57344`, one repeat, and no record cap. Relative to the exact committed 2048 no-adapt baseline, 65536 improves TEDLIUM and Earnings22, ties Rev16, and worsens CHiME-6 through higher deletion rate. No exact all-dataset 16384 no-adapt CTC baseline is committed for a full 2.7-minute comparison.
...
- ROB-122: Added a scoped ROB-81 Floras-finetuned encoder-decoder evaluation scaffold. The target checkpoint is `/store/store5/data/acp21rjf/symphony-job-artifacts/ROB-81/checkpoints/supervised_floras50_spotifytok_safe_norm_drop_oov_lr1e-4_12ep_nw0/step_323484.pt`; planned rows are matched no-adapt and `teacher_ce` one-epoch `beam5_lp0p5` thesis-setting evals for TEDLIUM, Earnings22, CHiME-6, and Rev16 test-only.
- ROB-122: The ROB-81 Floras-finetuned encoder-decoder evaluation completed 14 available rows under `lcasr/results/enc_dec/rob81_floras50_finetune_eval/`; Rev16 dev remains unavailable because the current loader exposes test only. Regenerated `summary.csv` and `OUTCOME.md` from the result PKLs. No-adapt/adapted WERs are TEDLIUM dev 8.54%/7.69%, TEDLIUM test 7.61%/7.11%, Earnings22 dev 28.86%/44.68%, Earnings22 test 20.83%/18.09%, CHiME-6 dev 81.51%/100.00%, CHiME-6 test 84.79%/100.00%, and Rev16 test 17.84%/68.92%.
