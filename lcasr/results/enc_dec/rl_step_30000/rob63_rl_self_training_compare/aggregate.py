...
LEGACY_CHECKPOINT_PATH = "/store/store5/data/acp21rjf_checkpoints/lcasr/enc_dec_v2/step_105360.pt"
...
def write_top_level_checkpoint_outcome(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    lines = [
        ...
        "|---|---|---|",
        f"| `enc_dec_v2/` | `enc_dec_v2` | Historical encoder-decoder checkpoint at `{LEGACY_CHECKPOINT_PATH}`; this is the older family with higher Earnings22 unadapted WER. |",
    ]
    for checkpoint, view_name in CHECKPOINT_VIEW_NAMES.items():
        lines.append(
    ...
    lines.extend(
        [
            ...
            f"  `{LEGACY_CHECKPOINT_PATH}`; use `old_seed/` and `rl_step_30000/` for",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional results/enc_dec root where old_seed/ and rl_step_30000/ views are written.",
    )
    parser.add_argument(
        "--top-level-outcome",
        type=Path,
