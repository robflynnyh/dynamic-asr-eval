"""Aggregate whole_iid_eval pickles into a dict keyed by setting.

For each setting (filename minus trailing `_<repeat>.pkl`):
- `baseline` and `adapted` are dataset-level WER dicts, computed corpus-wide
  over every original recording with the adapted weights,
...
def aggregate(directory: Path) -> dict:
    ...
    for setting, repeats in groups.items():
        ...
        out[setting] = {
            ...
            "adapt_num_chunks": repeats[0].get("adapt_num_chunks"),
        }
    return out
