"""Aggregate crossdataset pickles into a dict keyed by setting.

For each setting (filename minus trailing `_<repeat>.pkl`):
- `a_to_b` and `a_to_a_loo` are lists of WER dicts (one per adapt-recording
  in dataset A); these are averaged across adapt-recordings first to produce
  a single WER dict per repeat,
- `a_baseline` and `b_baseline` are taken as-is per repeat,
- all four are then averaged across repeats.
...
LIST_FIELDS = ("a_to_b", "a_to_a_loo")
BASELINE_FIELDS = ("a_baseline", "b_baseline")
