...
def get_lr_args_from_args(args):
    lr_args = {k.replace('lr_', ''):v for k,v in args.__dict__.items() if k.startswith('lr_')}
...
def dynamic_eval_ctc_loss(
    ...
    for epoch in range(args.__dict__.get('epochs', 1)):
        ...
        for i in pbar:
            ...
            #audio_chunk[:num_negatives] = replace_with_frame(audio_chunk[:num_negatives]) # apply augmentation to 2 of the 3 copies
            ...
            # for p, o in zip(model.parameters(), original_model_params):
            #     p.data.copy_(o)
            # #optimizer = optim(model.parameters(), **lr_args)
