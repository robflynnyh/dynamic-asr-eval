...
RESULTS_DIR="./results/whole_iid_eval"
...
do
    ...
    for epoch in "${EPOCHS[@]}"
    do
        SAVE_PATH="${RESULTS_DIR}/${DATASET}-${SPLIT}-whole-iid-epoch-${epoch}-lr-${LR_TAG}.pkl"
        LOG_PATH="${LOG_DIR}/${DATASET}-${SPLIT}-whole-iid-epoch-${epoch}-lr-${LR_TAG}.log"

        echo "[$(date -u +'%Y-%m-%dT%H:%M:%SZ')] starting whole-iid eval epoch=${epoch} lr=${lr}" | tee -a "$LOG_PATH"
        echo "save_path=$SAVE_PATH" | tee -a "$LOG_PATH"
        echo "log_path=$LOG_PATH" | tee -a "$LOG_PATH"

        CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON_BIN" run_whole_iid_eval.py \
            -dfa \
            -epochs "$epoch" \
            -seq "$SEQ" \
            -o "$OVERLAP" \
