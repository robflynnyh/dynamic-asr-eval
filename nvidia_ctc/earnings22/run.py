...
def main(args):
    ...
    model = lib.load_pretrained_model(args.checkpoint)
    ...
    for rec in tqdm(range(len(meetings_keys)), total=len(audio_files)):
        ...
        audio_spec = lib.preprocess(cur_audio, model)
        
        print('\n-------\n'+cur_meetings+'\n-------\n')
        
        logits = lib.dynamic_eval(args, model, audio_spec, args.seq_len, args.overlap, tokenizer)
