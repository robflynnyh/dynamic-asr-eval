...
RESULTS_PATH="${RESULTS_PATH:-lcasr/results/enc_dec/rob63_earnings_unadapted_sanity}"
...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 Earnings22 unadapted sanity check exited. Inspect \`lcasr/results/enc_dec/rob63_earnings_unadapted_sanity/OUTCOME.md\`, \`summary.csv\`, pickles, and logs before finalizing; this reruns beam5/lp0.5 unadapted Earnings22 for \`old_seed\` and the older \`enc_dec/OUTCOME\` checkpoint." \
    "${callback_args[@]}"
  callback_status=$?
...
RESULTS_ROOT=./results/enc_dec/rob63_earnings_unadapted_sanity \
