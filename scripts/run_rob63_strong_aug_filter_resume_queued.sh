...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 stronger-augmentation/filter resume completed. Inspect \`lcasr/results/enc_dec/rob63_strong_aug_filter_followup/OUTCOME.md\`, \`summary.csv\`, pickles, and logs before finalizing; this resume targets only the missing Rev16 RL \`3e-8\` no-filter/basic-filter cells." \
    "${callback_args[@]}"
  callback_status=$?
