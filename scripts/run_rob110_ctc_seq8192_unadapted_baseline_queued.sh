...
SCREEN_NAME=${SCREEN_NAME:-rob110_ctc_seq8192_unadapted_baseline}
...
QUEUED_COMMAND=${QUEUED_COMMAND:-"/store/store5/software/simple-gpu-schedule/with-gpu 1,2 -- bash scripts/run_rob110_ctc_seq8192_unadapted_baseline_queued.sh"}
...
SCRATCH_ROOT=${SCRATCH_ROOT:-"${REPO_ROOT}/symphony/.scratch/rob110_seq8192"}
...
on_exit() {
  ...
  callback_args=(
    ...
    --note "ROB-110 human follow-up: unadapted/no-adapt 8192-context CTC repeat-1 baseline over Earnings-22, TEDLIUM, CHiME-6, and Rev16. The wrapper runs epochs=0 evals, regenerates summary.csv/summary.md, and refreshes the 2048/8192/16384/65536 no-adapt comparison table when all 8192 artifacts exist."
  )
...
{
  echo "[$(date -u +'%Y-%m-%dT%H:%M:%SZ')] ROB-110 8192 unadapted baseline wrapper starting"
