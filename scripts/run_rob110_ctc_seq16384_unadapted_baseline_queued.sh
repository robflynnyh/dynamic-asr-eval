...
SCREEN_NAME=${SCREEN_NAME:-rob110_ctc_seq16384_unadapted_baseline}
RESULTS_PATH=${RESULTS_PATH:-"${REPO_ROOT}/lcasr/results/ctc_seq16384_unadapted_baseline"}
LOG_PATH=${LOG_PATH:-"${RESULTS_PATH}/logs/${SCREEN_NAME}.log"}
RUNNER_LABEL=${RUNNER_LABEL:-"screen:${SCREEN_NAME}"}
QUEUED_COMMAND=${QUEUED_COMMAND:-"/store/store5/software/simple-gpu-schedule/with-gpu 1,2 -- bash scripts/run_rob110_ctc_seq16384_unadapted_baseline_queued.sh"}
...
SCRATCH_ROOT=${SCRATCH_ROOT:-"${REPO_ROOT}/symphony/.scratch/rob110"}
...
on_exit() {
  ...
  callback_args=(
    ...
    --note "ROB-110 unadapted/no-adapt 16384-context CTC baseline over Earnings-22, TEDLIUM, CHiME-6, and Rev16. The wrapper runs epochs=0 evals, regenerates summary.csv/summary.md, and writes a 2048/16384/65536 no-adapt comparison table when all 16384 artifacts exist."
  )
...
{
  echo "[$(date -u +'%Y-%m-%dT%H:%M:%SZ')] ROB-110 unadapted baseline wrapper starting"
...
COMMON_ENV=(
  ...
  RESULTS_DIR="./results/ctc_seq16384_unadapted_baseline"
...
env "${COMMON_ENV[@]}" bash launch_scripts/run_ctc_seq16384_unadapted_baseline.sh 2>&1 | tee -a "$LOG_PATH"

cd "$REPO_ROOT"
python lcasr/results/ctc_seq16384_unadapted_baseline/aggregate.py 2>&1 | tee -a "$LOG_PATH"
python lcasr/results/ctc_seq16384_unadapted_baseline/compare_context_baselines.py 2>&1 | tee -a "$LOG_PATH"
