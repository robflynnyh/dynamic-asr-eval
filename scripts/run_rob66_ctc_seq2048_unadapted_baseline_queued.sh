...
REPO_ROOT=${REPO_ROOT:-"/exp/exp4/acp21rjf/symphony-workspaces-dynamic-asr-eval/ROB-66"}
...
LINEAR_ISSUE=${LINEAR_ISSUE:-ROB-66}
SCREEN_NAME=${SCREEN_NAME:-rob66_ctc_seq2048_unadapted_baseline}
RESULTS_PATH=${RESULTS_PATH:-"${REPO_ROOT}/lcasr/results/ctc_seq2048_unadapted_baseline"}
LOG_PATH=${LOG_PATH:-"${RESULTS_PATH}/logs/${SCREEN_NAME}.log"}
RUNNER_LABEL=${RUNNER_LABEL:-"screen:${SCREEN_NAME}"}
QUEUED_COMMAND=${QUEUED_COMMAND:-"/store/store5/software/simple-gpu-schedule/with-gpu 1,2 -- bash scripts/run_rob66_ctc_seq2048_unadapted_baseline_queued.sh"}
...
on_exit() {
  ...
  callback_args=(
    ...
    --note "ROB-66 unadapted/no-adapt 2048-context CTC baseline over Earnings-22, TEDLIUM, CHiME-6, and Rev16. The wrapper runs epochs=0 evals and regenerates summary.csv plus summary.md for thesis consumption."
  )
...
{
  echo "[$(date -u +'%Y-%m-%dT%H:%M:%SZ')] ROB-66 unadapted baseline wrapper starting"
...
COMMON_ENV=(
  ...
  RESULTS_DIR="./results/ctc_seq2048_unadapted_baseline"
...
env "${COMMON_ENV[@]}" bash launch_scripts/run_ctc_seq2048_unadapted_baseline.sh 2>&1 | tee -a "$LOG_PATH"

cd "$REPO_ROOT"
python lcasr/results/ctc_seq2048_unadapted_baseline/aggregate.py 2>&1 | tee -a "$LOG_PATH"
