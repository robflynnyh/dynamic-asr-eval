...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 stronger-augmentation/filter follow-up completed. Inspect \`lcasr/results/enc_dec/checkpoint2/rob63_strong_aug_filter_followup/OUTCOME.md\`, \`summary.csv\`, pickles, and logs before finalizing; compare against the lower-LR and lighter-augmentation follow-ups." \
    "${callback_args[@]}"
  callback_status=$?
