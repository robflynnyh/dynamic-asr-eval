...
RESULTS_PATH="${RESULTS_PATH:-lcasr/results/enc_dec/rob63_rl_self_training_compare}"
...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 encoder-decoder self-training comparison wrapper completed. Inspect \`lcasr/results/enc_dec/rob63_rl_self_training_compare/OUTCOME.md\`, \`summary.csv\`, pickles, and logs before finalizing." \
    "${callback_args[@]}"
  callback_status=$?
