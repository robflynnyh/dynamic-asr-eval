...
RESULTS_PATH="${RESULTS_PATH:-lcasr/results/enc_dec/rob63_aug_followup}"
...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 augmentation follow-up completed. Inspect \`lcasr/results/enc_dec/rob63_aug_followup/OUTCOME.md\`, \`summary.csv\`, pickles, and logs before finalizing; compare against the lower-LR follow-up and CHiME-6 dev normal baselines." \
    "${callback_args[@]}"
  callback_status=$?
...
RESULTS_ROOT=./results/enc_dec/rob63_aug_followup \
