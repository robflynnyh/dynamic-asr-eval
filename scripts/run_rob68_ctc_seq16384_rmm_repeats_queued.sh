...
SCREEN_NAME=${SCREEN_NAME:-rob68_ctc_seq16384_rmm_repeats23}
RESULTS_PATH=${RESULTS_PATH:-"${REPO_ROOT}/lcasr/results/ctc_seq16384_rmm_eval"}
LOG_PATH=${LOG_PATH:-"${RESULTS_PATH}/logs/${SCREEN_NAME}.log"}
RUNNER_LABEL=${RUNNER_LABEL:-"screen:${SCREEN_NAME}"}
QUEUED_COMMAND=${QUEUED_COMMAND:-"/store/store5/software/simple-gpu-schedule/with-gpu 1,2 -- bash scripts/run_rob68_ctc_seq16384_rmm_repeats_queued.sh"}
...
on_exit() {
  ...
  callback_args=(
    ...
    --note "ROB-68 follow-up: fill 16384-context RMM repeats 2 and 3 so each dataset/epoch setting has 3 repeats total. Inspect lcasr/results/ctc_seq16384_rmm_eval summary files and logs before finalizing."
  )
...
{
  echo "[$(date -u +'%Y-%m-%dT%H:%M:%SZ')] ROB-68 16384-context RMM repeat-fill wrapper starting"
  ...
  echo "repeat_indices=${REPEAT_INDICES:-2 3}"
  echo "total_repeats=${TOTAL_REPEATS:-3}"
  echo "lr=${LR:-9e-5}"
} | tee -a "$LOG_PATH"
...
COMMON_ENV=(
  ...
  RESULTS_DIR="./results/ctc_seq16384_rmm_eval"
  ...
  REPEATS=${REPEATS:-3}
  TOTAL_REPEATS=${TOTAL_REPEATS:-3}
  REPEAT_INDICES=${REPEAT_INDICES:-"2 3"}
...
cd "$REPO_ROOT"
python lcasr/results/ctc_seq16384_rmm_eval/aggregate.py 2>&1 | tee -a "$LOG_PATH"
