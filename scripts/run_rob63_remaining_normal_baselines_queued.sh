...
RESULTS_PATH="${RESULTS_PATH:-lcasr/results/enc_dec/rl_step_30000/rob61_checkpoint_benchmark}"
...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 remaining normal-baseline wrapper completed. Inspect \`lcasr/results/enc_dec/rl_step_30000/rob61_checkpoint_benchmark/OUTCOME.md\`, \`summary.csv\`, new CHiME-6/Rev16 normal pickles, and logs before regenerating ROB-63 summaries." \
    "${callback_args[@]}"
  callback_status=$?
