...
RESULTS_PATH="${RESULTS_PATH:-lcasr/results/enc_dec/rl_step_30000/rob63_lower_lr_dev_followup}"
...
on_exit() {
  ...
  python3 scripts/linear_experiment_callback.py \
    ...
    --commit "${GIT_COMMIT}" \
    --target-state Todo \
    --note "ROB-63 lower-LR follow-up completed. Inspect \`lcasr/results/enc_dec/rl_step_30000/rob63_lower_lr_dev_followup/OUTCOME.md\`, \`summary.csv\`, pickles, and logs; also inspect CHiME-6 dev normal baselines under \`lcasr/results/enc_dec/rl_step_30000/rob61_checkpoint_benchmark/\` before finalizing." \
    "${callback_args[@]}"
  callback_status=$?
...

{
  ...
  RESULTS_ROOT=./results/enc_dec/rl_step_30000/rob61_checkpoint_benchmark \
  ...
  RESULTS_ROOT=./results/enc_dec/rl_step_30000/rob63_lower_lr_dev_followup \
